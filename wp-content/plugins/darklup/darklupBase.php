<?php
if(!class_exists("darklupBase")) {
	class darklupBase {
    	public $key = "8B171655F251CD9D";
    	private $product_id = "1";
    	private $product_base = "darklup";
    	private $server_host = "https://auth.wpcommerz.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_darklup_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=darklup_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
